...
export function initRewardsTab() {
  // ── 유저에게 보내기: 닉네임 + 세그먼트(스킨/쪽지) + 확인창 + 미리보기 ──
  //  (감사 인사는 2026-08-30 제거 — 문구 고정이라 개인 쪽지의 부분집합이었음)
  ...
  function updateButtons() {
    const has = !!nickOf();
    [skinBtn, noteBtn].forEach(b => { if (b) b.disabled = !has; });
    if (revokeBtn) revokeBtn.disabled = !has;
  }

  // 닉네임 입력 → 스킨기본메시지 갱신 + 버튼 상태 + (열려 있으면)미리보기 즉시 반영
  nickInput.addEventListener('input', () => { syncSkinDefault(); updateButtons(); refreshPreviewIfOpen(); });
